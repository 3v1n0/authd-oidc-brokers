...
func TestSelectAuthenticationMode(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully select password":       {modeName: authmodes.PasswordID, tokenExists: true},
		"Successfully select device_auth_qr": {modeName: authmodes.DeviceQrID},
		"Successfully select newpassword":    {modeName: authmodes.NewPasswordID, secondAuthStep: true},

		"Selected newpassword shows correct label in passwd session": {modeName: authmodes.NewPasswordID, passwdSession: true, tokenExists: true, secondAuthStep: true},

		"Error when selecting invalid mode": {modeName: "invalid", wantErr: true},
		"Error when selecting device_auth_qr but provider is unavailable": {modeName: authmodes.DeviceQrID, wantErr: true,
			customHandlers: map[string]testutils.ProviderHandler{
				"/device_auth": testutils.UnavailableHandler(),
			},
		},
		"Error when selecting device_auth_qr but request times out": {modeName: authmodes.DeviceQrID, wantErr: true,
			customHandlers: map[string]testutils.ProviderHandler{
...
func TestIsAuthenticated(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully authenticate user with password":           {firstMode: authmodes.PasswordID, preexistentToken: "valid"},

		"Authenticating with qrcode reacquires token":          {firstChallenge: "-", wantSecondCall: true, preexistentToken: "valid"},
		"Authenticating with password refreshes expired token": {firstMode: authmodes.PasswordID, preexistentToken: "expired"},
		"Authenticating with password still allowed if server is unreachable": {
			firstMode:        authmodes.PasswordID,
		...
		"Error when challenge can not be decrypted":         {firstMode: authmodes.PasswordID, badFirstKey: true},
		"Error when provided wrong challenge":               {firstMode: authmodes.PasswordID, preexistentToken: "valid", firstChallenge: "wrongpassword"},
		...
		"Error when mode is password and token does not exist": {firstMode: authmodes.PasswordID},
		...
		"Error when mode is password and token is invalid":                {firstMode: authmodes.PasswordID, preexistentToken: "invalid"},
		"Error when mode is password and cached token can't be refreshed": {firstMode: authmodes.PasswordID, preexistentToken: "no-refresh"},
		"Error when mode is password and token refresh times out": {firstMode: authmodes.PasswordID, preexistentToken: "expired",
			customHandlers: map[string]testutils.ProviderHandler{
				"/token": testutils.HangingHandler(broker.MaxRequestDuration + 1),
			},
		},
		"Error when mode is password and can not fetch user info": {firstMode: authmodes.PasswordID, preexistentToken: "invalid-id"},
		...
		"Error when mode is newpassword and token is not set": {
			firstMode:     authmodes.NewPasswordID,
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				if tc.firstMode == "" {
					tc.firstMode = authmodes.DeviceQrID
				}
			...
			if tc.wantSecondCall {
				...
				go func() {
					...
					updateAuthModes(t, b, sessionID, authmodes.NewPasswordID)
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				updateAuthModes(t, b, firstSession, authmodes.PasswordID)
			...
			go func() {
				...
				updateAuthModes(t, b, secondSession, authmodes.PasswordID)
...
func TestCancelIsAuthenticated(t *testing.T) {
	...
	updateAuthModes(t, b, sessionID, authmodes.DeviceQrID)
